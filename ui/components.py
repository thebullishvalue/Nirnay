...
ICONS = {
    ...
    "globe":      '<svg aria-label="Globe icon" role="img" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>',
    "alert-circle": '<svg aria-label="Alert icon" role="img" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>',
    ...
    "trending-down": '<svg aria-label="Trending down icon" role="img" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polyline points="23 18 13.5 8.5 8.5 13.5 1 6"/><polyline points="17 18 23 18 23 12"/></svg>',
    ...
    "monitor":    '<svg aria-label="Monitor icon" role="img" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>',
...
def render_metric_card(
    ...
    st.markdown(
        ...
        f'<div class="sub-metric">{html_mod.escape(subtext)}</div>' if subtext else ""
        f"{tooltip_html}"
        f"</div>",
        unsafe_allow_html=True,
    )
...
def premium_card(
    ...
    """Render a high-fidelity engine card with icon and description.

    Args:
        title: Card title (rendered uppercase).
        description: Primary explanation of the system engine.
        icon: Key from ICONS dict.
        accent: CSS color class — "cyan", "amber", "rose", "emerald", "violet".
    """
    svg = ICONS.get(icon, ICONS["cpu"])
    st.markdown(
        f"""
        <div class="premium-card {html_mod.escape(accent)}">
            <div class="header">
...
def interpretation_card(
    ...
    Args:
        ...
        subtitle: Quantitative range or condition (e.g. "Signal < -5").
    ...
    st.markdown(
        ...
        f'<div class="interp-subtitle">{html_mod.escape(subtitle)}</div>'
...
def render_interpretation_card(title: str, body: str, color: str = "neutral") -> None:
    ...
    interpretation_card(title, "", body, color)
