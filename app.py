"""
NIRNAY (िनणर्य) - Unified Market Analysis | A Pragyam Product Family Member
...
.main, [data-testid="stSidebar"] { background-color: var(--background-color); color: var(
.stApp > header { background-color: transparent; }
#MainMenu {visibility: hidden;} footer {visibility: hidden;}
.block-container { padding-top: 3.5rem; max-width: 90%; padding-left: 2rem; padding-right
...
background: radial-gradient(circle at 20% 50%, rgba(var(--primary-rgb),0.08) 0%, tran
pointer-events: none;
}
.premium-header h1 { margin: 0; font-size: 2rem; font-weight: 700; color: var(--text-prim
.premium-header .tagline { color: var(--text-muted); font-size: 0.9rem; margin-top: 0.25r
.premium-header .product-badge { display: inline-block; background: rgba(var(--primary-rg
.metric-card {
...
.metric-card h4 { color: var(--text-muted); font-size: 0.75rem; margin-bottom: 0.5rem; fo
.metric-card h2 { color: var(--text-primary); font-size: 1.75rem; font-weight: 700; margi
.metric-card .sub-metric { font-size: 0.75rem; color: var(--text-muted); margin-top: 0.5r
...
.signal-card::before { content: ''; position: absolute; top: 0; left: 0; width: 4px; heig
.signal-card.buy::before { background: var(--success-green); }
.signal-card.sell::before { background: var(--danger-red); }
.signal-card-header { display: flex; align-items: center; justify-content: space-between;
.signal-card-title { font-size: 0.8rem; font-weight: 700; text-transform: uppercase; lett
.status-badge { display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.4rem 0
.status-badge.buy { background: rgba(16, 185, 129, 0.15); color: var(--success-green); bo
.status-badge.sell { background: rgba(239, 68, 68, 0.15); color: var(--danger-red); borde
.status-badge.oversold { background: rgba(6, 182, 212, 0.15); color: var(--info-cyan); bo
.status-badge.overbought { background: rgba(245, 158, 11, 0.15); color: var(--warning-amb
.status-badge.neutral { background: rgba(136, 136, 136, 0.15); color: var(--neutral); bor
.status-badge.divergence { background: rgba(var(--primary-rgb), 0.15); color: var(--prima
.info-box { background: var(--secondary-background-color); border: 1px solid var(--border
.info-box h4 { color: var(--primary-color); margin: 0 0 0.5rem 0; font-size: 1rem; .info-box p { color: var(--text-muted); margin: 0; font-size: 0.9rem; line-height: font-w
1.6; }
.stButton>button { border: 2px solid var(--primary-color); background: transparent; color
.stButton>button:hover { box-shadow: 0 0 25px rgba(var(--primary-rgb), 0.6); background:
.stButton>button:active { transform: translateY(0); }
.stTabs [data-baseweb="tab-list"] { gap: 24px; background: transparent; }
.stTabs [data-baseweb="tab"] { color: var(--text-muted); border-bottom: 2px solid transpa
.stTabs [aria-selected="true"] { color: var(--primary-color); border-bottom: 2px solid va
.stPlotlyChart { border-radius: 12px; background-color: var(--secondary-background-color)
.stDataFrame { border-radius: 12px; background-color: var(--secondary-background-color);
.section-divider { height: 1px; background: linear-gradient(90deg, transparent 0%, var(--
.symbol-row { display: flex; align-items: center; justify-content: space-between; padding
...
.conviction-meter { height: 8px; background: var(--bg-elevated); border-radius: 4px; over
.conviction-fill { height: 100%; border-radius: 4px; transition: width 0.3s ease; }
.sidebar-title { font-size: 0.75rem; font-weight: 700; color: var(--primary-color); text-
[data-testid="stSidebar"] { background: var(--secondary-background-color); border-right:
.stTextInput > div > div > input { background: var(--bg-elevated) !important; border: 1px
.stTextInput > div > div > input:focus { border-color: var(--primary-color) !important; b
...
"EU (DE) 30Y": "30YDEY.B", "EU (DE) 10Y": "10YDEY.B", "EU (DE) 05Y": "5YDEY.B", "EU (DE)
...
symbols = df['Symbol'].str.strip().str.replace('.', '-', regex=False).tol
...
response.raise_for_status()
tables = pd.read_html(io.StringIO(response.text))
# Find the table with ticker symbols (usually has 'Symbol' or 'Ticker' for tbl in tables:
for col in ['Symbol', 'Ticker', 'Ticker symbol']:
if col in tbl.columns:
symbols = tbl[col].str.strip().tolist()
symbols = [s for s in symbols if s and str(s).strip() and len(s)
if 20 <= len(symbols) <= 35:
return symbols, f"✓ Fetched {len(symbols)} Dow Jones constitu
except Exception:
pass
# Fallback to hardcoded list
return DOW_JONES_TICKERS.copy(), f"✓ Loaded {len(DOW_JONES_TICKERS)} Dow Jones co
...
symbols = tbl['Ticker'].str.strip().str.replace('.', '-', regex=False).to
symbols = [s for s in symbols if s and str(s).strip()]
if len(symbols) >= 90:
return symbols, f"✓ Fetched {len(symbols)} NASDAQ 100 constituents"
elif 'Symbol' in tbl.columns:
symbols = tbl['Symbol'].str.strip().str.replace('.', '-', regex=False).to
symbols = [s for s in symbols if s and str(s).strip()]
if len(symbols) >= 90:
return symbols, f"✓ Fetched {len(symbols)} NASDAQ 100 constituents"
...
data_dict = {t:df.copy() for t,df in all_data.items() if not df.empty and not df[
else:
...
if isinstance(live_data, pd.DataFrame) and isinstance(live_data.colum
for ticker in data_dict.keys():
try:
live_ticker = live_data.xs(ticker, level=0, axis=1)
if not live_ticker.empty and not live_ticker['Close'].isn
...
live_ticker.index = live_ticker.index.tz_localize
# Only append if not already present
new_dates = live_ticker.index.difference(hist_df.inde
if len(new_dates) > 0:
data_dict[ticker] = pd.concat([hist_df, live_tick
except KeyError:
pass
return data_dict, f"✓ Downloaded {len(data_dict)} tickers (with live
except Exception:
...
try:
url = f"https://stooq.com/q/d/l/?s={symbol}&d1={start_date.strftime('%Y%m%d')}&d2={en
...
if len(new_dates) > 0:
target_df = pd.concat([target_df, live_df.loc[new_dates]]).sort_i
except Exception:
pass
...
return _core_run_full_analysis(df, length, roc_len, regime_sensitivity, base_weight, MACR
# ══════════════════════════════════════════════════════════════════════════════
# CHART FUNCTIONS
# ══════════════════════════════════════════════════════════════════════════════
...
fig.add_trace(go.Scatter(x=df.index, y=ma20, mode='lines', name='MA20', line=dict(color='
ma50 = df['Close'].rolling(50).mean()
fig.add_trace(go.Scatter(x=df.index, y=ma50, mode='lines', name='MA50', line=dict(color='
fig.update_layout(
template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='#1A1A1A', margin=dict(l=10, r=10, t=30, b=50),
xaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)', rangeslider=dict(visible=Fa
yaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)', side='right'),
legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, bgcolor=
...
fig = go.Figure()
fig.add_trace(go.Scatter(x=df.index, y=df['Unified_Osc'].clip(lower=0), fill='tozeroy', f
fig.add_trace(go.Scatter(x=df.index, y=df['Unified_Osc'].clip(upper=0), fill='tozeroy', f
trace_colors = np.where(df['Unified_Osc'] < -5, '#10b981', np.where(df['Unified_Osc'] > 5
fig.add_trace(go.Scatter(x=df.index, y=df['Unified_Osc'], mode='lines+markers', name='Uni
fig.add_trace(go.Scatter(x=df.index, y=df['MSF_Osc'], mode='lines', name='MSF (Internal)'
fig.add_trace(go.Scatter(x=df.index, y=df['MMR_Osc'], mode='lines', name='MMR (Macro)', l
buys = df[df['Buy_Signal']]
if not buys.empty:
fig.add_trace(go.Scatter(x=buys.index, y=buys['Unified_Osc'], mode='markers', name='B
sells = df[df['Sell_Signal']]
if not sells.empty:
fig.add_trace(go.Scatter(x=sells.index, y=sells['Unified_Osc'], mode='markers', name=
...
height
fig.update_layout(
template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='#1A1A1A', margin=dict(l=10, r=10, t=10, b=50),
xaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)'),
yaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)', range=[-12, 12], tickvals=[
legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, bgcolor=
font=dict(family='Inter', color='#EAEAEA'), hovermode='x unified'
)
return fig
...
gauge=dict(
axis=dict(range=[-10, 10], tickwidth=1, tickcolor='#3A3A3A', tickvals=[-10, -5, 0
bar=dict(color=color, thickness=0.3), bgcolor='#1A1A1A', borderwidth=2, bordercol
steps=[dict(range=[-10, -5], color='rgba(16,185,129,0.15)'), dict(range=[-5, 5],
...
colorscale = [[0, '#10b981'], [0.25, '#059669'], [0.5, '#1A1A1A'], [0.75, '#dc2626'], [1,
normalized_scores = (scores_grid + 10) / 20
fig = go.Figure(data=go.Heatmap(
z=normalized_scores,
text=[[f"{s}<br>{v:.1f}" if s else "" for s, v in zip(row_s, row_v)] for row_s, row_v
texttemplate="%{text}", textfont=dict(size=11, color='white', family='Inter'),
colorscale=colorscale, showscale=False, hovertemplate="<b>%{text}</b><extra></extra>"
))
fig.update_layout(
...
fig = go.Figure()
fig.add_trace(go.Histogram(x=results_df['Signal'], nbinsx=20, marker=dict(color='#FFC300'
...
fig.update_layout(
template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='#1A1A1A', margin=dict(l=0, r=0, t=10, b=30),
xaxis=dict(title=dict(text='Signal Value', font=dict(size=10, color='#888888')), show
yaxis=dict(title=dict(text='Count', font=dict(size=10, color='#888888')), showgrid=Tr
...
sectors = {
'Index': ['SENSEX', 'NIFTY 50', 'NIFTY 100', 'NIFTY 500', 'Top 50', 'Midcap', 'Smallc
...
fig = go.Figure()
fig.add_trace(go.Scatterpolar(r=values, theta=categories, fill='toself', fillcolor='rgba(
fig.update_layout(
polar=dict(
radialaxis=dict(visible=True, range=[-10, 10], tickvals=[-10, -5, 0, 5, 10], grid
angularaxis=dict(gridcolor='rgba(42,42,42,0.5)', linecolor='rgba(42,42,42,0.5)',
bgcolor='#1A1A1A'
),
paper_bgcolor='rgba(0,0,0,0)', height=350, margin=dict(l=60, r=60, t=30, b=30), font=
)
return fig
def create_scatter_matrix(results_df):
fig = go.Figure()
colors = results_df['Zone'].map({'Oversold': '#10b981', 'Overbought': '#ef4444', 'Neutral
...
text=results_df['DisplayName'], textposition='top center', textfont=dict(size=8, colo
...
fig.add_annotation(x=6, y=6, text="SELL ZONE", font=dict(size=10, color='rgba(239,68,68,0
fig.add_annotation(x=-6, y=-6, text="BUY ZONE", font=dict(size=10, color='rgba(16,185,129
fig.update_layout(
template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='#1A1A1A', margin=dict(l=40, r=10, t=30, b=40),
xaxis=dict(title=dict(text='MSF (Internal)', font=dict(size=11, color='#888888')), sh
yaxis=dict(title=dict(text='MMR (Macro)', font=dict(size=11, color='#888888')), showg
...
text=[f"{v:.1f}" for v in combined['Signal']], textposition='outside', textfont=dict(
))
...
fig.update_layout(
template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='#1A1A1A', margin=dict(l=80, r=50, t=10, b=10),
xaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)', range=[-12, 12], tickvals=[
...
<div style="color: #888888; font-size: 0.75rem; margin-top: 0.25rem;">िनणर्य | Decis
</div>
""", unsafe_allow_html=True)
st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
mode = st.radio("Analysis Mode", [" Home", " ETF Screener", " Market Screener"
st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
...
etf_end_date = None
if "ETF" in mode:
st.markdown('<div class="sidebar-title"> Analysis Type</div>', unsafe_allow_htm
etf_mode = st.radio(
"Select ETF Mode",
[" Single Day", " Time Series"],
label_visibility="collapsed",
help="Single Day: Analyze one date | Time Series: Track signals over a date r
)
if "Single" in etf_mode:
st.markdown('<div class="sidebar-title"> Analysis Date</div>', unsafe_allow
...
st.markdown('<div class="sidebar-title"> Date Range</div>', unsafe_allow_ht
...
if "Market" in mode:
st.markdown('<div class="sidebar-title"> Universe Selection</div>', unsafe_allo
...
st.markdown(f"""
<div class='info-box'>
<p style='font-size: 0.8rem; margin: 0; color: var(--text-muted); line-height: 1.
...
""", unsafe_allow_html=True)
return mode, length, roc_len, regime_sensitivity, base_weight, spread_universe, sprea
def run_home_page():
...
Full MSF + MMR + Regime analysis across a curated universe of 30 ETFs coverin
</p>
<br>
...
<h3 style='color: var(--success-green); margin-bottom: 1rem;'> Market Screener<
<p style='color: var(--text-muted); font-size: 0.9rem; line-height: 1.6;'>
MSF-based signal analysis across F&O stocks or index constituents. Scan 200-5
</p>
<br>
<p style='color: var(--text-secondary); font-size: 0.85rem;'>
<strong>Features:</strong><br>
...
<p style='color: var(--text-muted); font-size: 0.9rem; line-height: 1.6;'>
Deep dive into individual securities with unified analysis including regime c
</p>
<br>
<p style='color: var(--text-secondary); font-size: 0.85rem;'>
<strong>Features:</strong><br>
...
<h4 style='color: var(--success-green); margin-bottom: 1rem;'>MSF - Market Struct
<p style='color: var(--text-muted); font-size: 0.85rem; line-height: 1.7;'>
Internal price-based indicator:
</p>
<ul style='color: var(--text-secondary); font-size: 0.85rem; line-height: 1.8; ma
...
<h4 style='color: var(--danger-red); margin-bottom: 1rem;'>MMR - Macro Regression
<p style='color: var(--text-muted); font-size: 0.85rem; line-height: 1.7;'>
...
with col_s1:
st.markdown("""
<div style='background: rgba(16, 185, 129, 0.1); border: 1px solid var(--success-gree
<h4 style='color: var(--success-green); margin-bottom: 0.75rem;'> Oversold Zone
<p style='color: var(--text-muted); font-size: 0.85rem;'>Signal &lt; -5</p>
<p style='color: var(--text-secondary); font-size: 0.85rem; margin-top: 0.5rem;'>
Potential buying opportunity. Look for confirmation with divergences and macr
</p>
</div>
""", unsafe_allow_html=True)
with col_s2:
st.markdown("""
<div style='background: rgba(136, 136, 136, 0.1); border: 1px solid var(--neutral); b
...
<h4 style='color: var(--danger-red); margin-bottom: 0.75rem;'> Overbought Zone<
<p style='color: var(--text-muted); font-size: 0.85rem;'>Signal &gt; +5</p>
<p style='color: var(--text-secondary); font-size: 0.85rem; margin-top: 0.5rem;'>
Potential selling opportunity. Watch for bearish divergences and macro headwi
</p>
</div>
""", unsafe_allow_html=True)
st.markdown("<br>", unsafe_allow_html=True)
...
with c3:
with c4:
st.markdown(f'<div class="metric-card neutral"><h4>ETF Universe</h4><h2>{len(SCREENER
st.markdown(f'<div class="metric-card neutral"><h4>Index Options</h4><h2>{len(INDEX_L
st.markdown(f'<div class="metric-card neutral"><h4>Macro Factors</h4><h2>{len(MACRO_S
st.markdown(f'<div class="metric-card neutral"><h4>Analysis Modes</h4><h2>3</h2><div
st.markdown("<br>", unsafe_allow_html=True)
# Getting started
st.markdown("""
<div class='info-box'>
...
Select an analysis mode from the sidebar to begin. Each mode offers both <strong>
for current signals and <strong>Time Series</strong> analysis for tracking Adjust indicator parameters in the sidebar's <em>Indicator Settings</em> expander
signal
</p>
</div>
""", unsafe_allow_html=True)
def main():
mode, length, roc_len, regime_sensitivity, base_weight, spread_universe, spread_index, sp
...
run_etf_timeseries_mode(length, roc_len, regime_sensitivity, base_weight, etf_sta
else:
...
run_market_timeseries_mode(length, roc_len, regime_sensitivity, base_weight, spre
else:
run_market_screener_mode(length, roc_len, regime_sensitivity, base_weight, elif "Chart" in mode:
...
with col1:
target_symbol = st.text_input("Symbol", value="SPY", placeholder="Enter ticker with col2:
(e.g.,
...
try:
df, drivers = run_full_analysis(df, length, roc_len, regime_sensitivity,
...
with col1:
color_class = "success" if curr_condition == "Oversold" else "danger"
st.markdown(f'<div class="metric-card {color_class}"><h4>Unified Sign
with col2:
st.markdown(f'<div class="metric-card primary"><h4>MSF (Internal)</h4
with col3:
st.markdown(f'<div class="metric-card info"><h4>MMR (Macro)</h4><h2>{
with col4:
price_color = "success" if price_change >= 0 else "danger"
st.markdown(f'<div class="metric-card {price_color}"><h4>Price</h4><h
# Row 2: Regime Intelligence metrics (NEW)
st.markdown("<br>", unsafe_allow_html=True)
col1, col2, col3, col4 = st.columns(4)
with col1:
regime_color = "success" if "BULL" in curr_regime else "danger" if "B
st.markdown(f'<div class="metric-card {regime_color}"><h4>HMM Regime<
with col2:
with col3:
st.markdown(f'<div class="metric-card success"><h4>P(Bull)</h4><h2>{c
with col4:
vol_color = "danger" if curr_vol_regime in ["HIGH", "EXTREME"] else "
st.markdown(f'<div class="metric-card {vol_color}"><h4>Volatility Reg
st.markdown(f'<div class="metric-card danger"><h4>P(Bear)</h4><h2>{cu
# Change point alert (NEW)
if curr_change_point:
st.markdown('<span class="status-badge regime"> CHANGE POINT DETECT
if has_buy or has_sell:
st.markdown("<br>", unsafe_allow_html=True)
if has_buy:
st.markdown('<span class="status-badge buy">◉ CONFIRMED BUY SIGNA
if has_sell:
st.markdown('<span class="status-badge sell">◉ CONFIRMED SELL SIG
st.markdown("<br>", unsafe_allow_html=True)
tab3, tab1, tab2 = st.tabs(["** Regime Intelligence**", "** Price & O
with tab3:
# Regime Intelligence Tab (FIRST)
st.markdown("##### HMM State Probabilities Over Time")
# Create HMM probability chart
fig_hmm = go.Figure()
fig_hmm.add_trace(go.Scatter(x=display_df.index, y=display_df['HMM_Bu
fig_hmm.add_trace(go.Scatter(x=display_df.index, y=display_df['HMM_Be
fig_hmm.add_hline(y=0.5, line_dash='dash', line_color='#888888')
fig_hmm.update_layout(template='plotly_dark', paper_bgcolor='rgba(0,0
st.plotly_chart(fig_hmm, width="stretch", config={'displayModeBar': F
c1, c2 = st.columns(2)
with c1:
st.markdown("##### Regime Timeline")
# Create regime color mapping
regime_colors = {'STRONG_BULL': '#059669', 'BULL': '#10b981', 'WE
regime_vals = display_df['Regime'].map(lambda x: regime_colors.ge
# Count regimes
regime_counts = display_df['Regime'].value_counts()
fig_regime = go.Figure(go.Pie(
labels=regime_counts.index, values=regime_counts.values, hole
marker=dict(colors=[regime_colors.get(r, '#888888') for r in
textinfo='label+percent', textfont=dict(size=10, color='white
))
fig_regime.update_layout(paper_bgcolor='rgba(0,0,0,0)', font=dict
st.plotly_chart(fig_regime, width="stretch", config={'displayMode
with c2:
st.markdown("##### Volatility Regime Distribution")
vol_counts = display_df['Vol_Regime'].value_counts()
vol_colors = {'LOW': '#10b981', 'NORMAL': '#888888', 'HIGH': '#f5
fig_vol = go.Figure(go.Pie(
labels=vol_counts.index, values=vol_counts.values, hole=0.5,
marker=dict(colors=[vol_colors.get(v, '#888888') for v textinfo='label+percent', textfont=dict(size=10, color='white
in vol
))
fig_vol.update_layout(paper_bgcolor='rgba(0,0,0,0)', font=dict(fa
st.plotly_chart(fig_vol, width="stretch", config={'displayModeBar
# Change points
change_points = display_df[display_df['Change_Point'] == True]
if len(change_points) > 0:
st.markdown(f"##### Change Points Detected: {len(change_points)}"
st.dataframe(change_points[['Close', 'Unified_Osc', 'Regime', 'Vo
else:
st.info("No change points detected in this period")
with tab1:
st.plotly_chart(create_price_chart(display_df, target_symbol), width=
st.plotly_chart(create_oscillator_chart(display_df), width="stretch",
with tab2:
c1, c2 = st.columns(2)
with c1:
st.markdown("##### Signal Gauge")
st.plotly_chart(create_gauge_chart(curr_unified), width="stretch"
with c2:
st.markdown("##### Top Macro Drivers")
if drivers:
for d in sorted(drivers, key=lambda x: abs(x['Correlation']),
corr = d['Correlation']
color = '#10b981' if corr > 0 else '#ef4444'
pct = abs(corr) * 100
st.markdown(f'<div style="margin-bottom: 0.75rem;"><div s
else:
...
<strong>Analysis Date:</strong> {analysis_date_str} {"(Today)" if is_today else ""}</
</div>
""", unsafe_allow_html=True)
st.markdown("<br>", unsafe_allow_html=True)
...
try:
df, _ = run_full_analysis(df, length, roc_len, regime_sensitivity, base_w
...
signal_str = "BUY" if last_row['Buy_Signal'] else "SELL" if last_row['Sel
div_str = "BULL" if last_row['Bullish_Div'] else "BEAR" if last_row['Bear
results.append({
...
status_text.empty()
if errors:
st.warning(f" {len(errors)}/{total} tickers failed analysis. Sample: {errors[0]
if results:
st.toast(f"ETF Scan Complete! Analyzed {len(results)}/{total} ETFs", icon=" results_df = pd.DataFrame(results)
")
...
dominant_regime = results_df['Regime'].mode().iloc[0] if len(results_df) > regime_color = "success" if "BULL" in dominant_regime else "danger" if "BEAR" in
0 else
# Metrics row
st.markdown("<br>", unsafe_allow_html=True)
c1, c2, c3, c4, c5, c6 = st.columns(6)
with c1:
st.markdown(f'<div class="metric-card info"><h4>Universe</h4><h2>{len(results
with c2:
st.markdown(f'<div class="metric-card success"><h4>Oversold</h4><h2>{n_overso
with c3:
st.markdown(f'<div class="metric-card danger"><h4>Overbought</h4><h2>{n_overb
with c4:
st.markdown(f'<div class="metric-card primary"><h4>Buy Signals</h4><h2>{n_buy
with c5:
st.markdown(f'<div class="metric-card warning"><h4>Sell Signals</h4><h2>{n_se
with c6:
st.markdown(f'<div class="metric-card {regime_color}"><h4>HMM Regime</h4><h2
st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
# # Tabs for ETF Screener - Regime Analysis first
tab4, tab1, tab2, tab3, tab5 = st.tabs(["** Regime Analysis**", "** Signal Da
with tab1:
col_buy, col_sell = st.columns(2)
with col_buy:
st.markdown('<div class="signal-card buy"><div class="signal-card-header"
confirmed_buys = results_df[results_df['Trigger'] == 'BUY'].sort_values('
if not confirmed_buys.empty:
st.markdown('<span class="status-badge buy">CONFIRMED BUY SIGNALS</sp
for _, row in confirmed_buys.iterrows():
st.markdown(f'<div class="symbol-row"><div><span class="symbol-na
st.markdown("<br>", unsafe_allow_html=True)
oversold = results_df[(results_df['Zone'] == 'Oversold') & (results_df['T
if not oversold.empty:
st.markdown('<span class="status-badge oversold">OVERSOLD ZONE</span>
for _, row in oversold.iterrows():
st.markdown(f'<div class="symbol-row"><div><span class="symbol-na
if confirmed_buys.empty and oversold.empty:
st.markdown('<p style="color: #888888; padding: 1rem;">No buy opportu
st.markdown("</div>", unsafe_allow_html=True)
with col_sell:
st.markdown('<div class="signal-card sell"><div class="signal-card-header
confirmed_sells = results_df[results_df['Trigger'] == 'SELL'].sort_values
if not confirmed_sells.empty:
st.markdown('<span class="status-badge sell">CONFIRMED SELL SIGNALS</
for _, row in confirmed_sells.iterrows():
st.markdown(f'<div class="symbol-row"><div><span class="symbol-na
st.markdown("<br>", unsafe_allow_html=True)
overbought = results_df[(results_df['Zone'] == 'Overbought') & (results_d
if not overbought.empty:
st.markdown('<span class="status-badge overbought">OVERBOUGHT ZONE</s
for _, row in overbought.iterrows():
st.markdown(f'<div class="symbol-row"><div><span class="symbol-na
if confirmed_sells.empty and overbought.empty:
st.markdown('<p style="color: #888888; padding: 1rem;">No sell st.markdown("</div>", unsafe_allow_html=True)
opport
# Divergence alerts
st.markdown("<br>", unsafe_allow_html=True)
...
st.markdown('<span class="status-badge divergence">BULLISH for _, row in bull_divs.head(10).iterrows():
st.markdown(f'<div class="symbol-row"><span class="symbol-nam
with div_cols[1]:
if not bear_divs.empty:
st.markdown('<span class="status-badge divergence">BEARISH for _, row in bear_divs.head(10).iterrows():
st.markdown(f'<div class="symbol-row"><span class="symbol-nam
DIVERG
with tab2:
st.markdown("##### Top Oversold ETFs")
top_oversold = results_df.nsmallest(15, 'Signal')
cols_o = ['DisplayName', 'Price', 'Change', 'Signal', 'MSF', 'MMR', 'Zone', '
st.dataframe(top_oversold[cols_o].rename(columns={'DisplayName': 'ETF', 'Chan
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("##### Top Overbought ETFs")
top_overbought = results_df.nlargest(15, 'Signal')
st.dataframe(top_overbought[cols_o].rename(columns={'DisplayName': 'ETF', 'Ch
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("##### Signal Ranking Chart")
st.plotly_chart(create_ranking_chart(results_df, 15), width="stretch", config
with tab3:
col_d1, col_d2 = st.columns(2)
with col_d1:
st.markdown("##### Signal Distribution")
st.plotly_chart(create_distribution_chart(results_df), width="stretch", c
st.markdown("##### Zone Breakdown")
zone_data = {
"Zone": ["Oversold (< -5)", "Neutral (-5 to +5)", "Overbought (> +5)"
"Count": [n_oversold, n_neutral, n_overbought],
"Percentage": [f"{n_oversold/len(results_df)*100:.1f}%", f"{n_neutral
}
st.dataframe(pd.DataFrame(zone_data), width="stretch", hide_index=True)
with col_d2:
st.markdown("##### Statistical Summary")
stats_data = {
"Metric": ["Total ETFs", "Mean Signal", "Median Signal", "Std Dev", "
"Value": [
f"{len(results_df)}",
...
top_gainers = results_df.nlargest(10, 'Change')[['DisplayName', 'Price',
...
top_losers = results_df.nsmallest(10, 'Change')[['DisplayName', 'Price',
...
regime_colors = {'STRONG_BULL': '#059669', 'BULL': '#10b981', 'WEAK_BULL'
fig_regime = go.Figure(go.Pie(
labels=regime_counts.index, values=regime_counts.values, hole=0.5,
marker=dict(colors=[regime_colors.get(r, '#888888') for r in regime_c
textinfo='label+percent', textfont=dict(size=11, color='white')
))
fig_regime.update_layout(paper_bgcolor='rgba(0,0,0,0)', font=dict(family=
st.plotly_chart(fig_regime, width="stretch", config={'displayModeBar': Fa
with c2:
vol_counts = results_df['Vol_Regime'].value_counts()
vol_colors = {'LOW': '#10b981', 'NORMAL': '#888888', 'HIGH': '#f59e0b', '
fig_vol = go.Figure(go.Pie(
labels=vol_counts.index, values=vol_counts.values, hole=0.5,
marker=dict(colors=[vol_colors.get(v, '#888888') for v in vol_counts.
textinfo='label+percent', textfont=dict(size=11, color='white')
))
fig_vol.update_layout(paper_bgcolor='rgba(0,0,0,0)', font=dict(family='In
st.plotly_chart(fig_vol, width="stretch", config={'displayModeBar': False
st.markdown("##### Regime Breakdown by ETF")
# Bull regime ETFs
col_bull, col_bear = st.columns(2)
with col_bull:
st.markdown('<span class="status-badge buy">BULLISH REGIME ETFs</span>',
bull_etfs = results_df[results_df['Regime'].str.contains('BULL', na=False
if len(bull_etfs) > 0:
...
with col_bear:
st.markdown('<span class="status-badge sell">BEARISH REGIME ETFs</span>',
bear_etfs = results_df[results_df['Regime'].str.contains('BEAR', na=False
if len(bear_etfs) > 0:
...
st.markdown("<br>", unsafe_allow_html=True)
st.markdown('<span class="status-badge regime"> CHANGE POINTS DETECTED<
for _, row in change_point_etfs.iterrows():
st.markdown(f'<div class="symbol-row"><div><span class="symbol-name">
with tab5:
st.markdown(f"##### Complete ETF Scan Results ({len(results_df)} ETFs) - {ana
# Filter options
filter_col1, filter_col2, filter_col3 = st.columns(3)
with filter_col1:
zone_filter = st.multiselect("Filter by Zone", ["Oversold", "Neutral", "O
with filter_col2:
signal_filter = st.multiselect("Filter by Trigger", ["BUY", "SELL", "-"],
with filter_col3:
sort_by = st.selectbox("Sort by", ["Signal", "Change", "Price", "DisplayN
# Apply filters
...
display_cols = ['DisplayName', 'Price', 'Change', 'Signal', 'Zone', 'Trigger'
display_df = filtered_df[display_cols].copy()
display_df.columns = ['ETF', 'Price', 'Chg %', 'Signal', 'Zone', 'Trigger', '
...
def run_market_screener_mode(length, roc_len, regime_sensitivity, base_weight, spread_univers
...
<p>Full NIRNAY (MSF + MMR + Regime) analysis across all F&O securities from NSE.<
<strong>Analysis Date:</strong> {analysis_date_str} {"(Today)" if is_today else "
</div>
""", unsafe_allow_html=True)
else:
...
<div class='info-box'>
<h4> Market Screener - {universe_title}</h4>
<p>Full NIRNAY (MSF + MMR + Regime) analysis across all constituents of {universe
<strong>Analysis Date:</strong> {analysis_date_str} {"(Today)" if is_today else "
</div>
""", unsafe_allow_html=True)
st.markdown("<br>", unsafe_allow_html=True)
...
# Batch download data
status_text.markdown(f"** progress_bar.progress(0.05)
Downloading data for {total_stocks} stocks...**")
data_dict, batch_msg = fetch_batch_data(stock_list, end_date=analysis_date, days_back
...
status_text.markdown(f"** Analyzing {ticker.replace('.NS', '')} ({i+1}/{total_v
...
"Signal": round(last_row['Unified_Osc'], 2),
"MSF": round(last_row['MSF_Osc'], 2),
"MMR": round(last_row['MMR_Osc'], 2), # Now populated with actual MM
"Zone": last_row['Condition'],
"Trigger": signal_str,
"Divergence": div_str,
"Agreement": round(last_row['Agreement'], 3),
...
status_text.empty()
if errors:
st.warning(f" {len(errors)}/{total_valid} tickers failed analysis. Sample: {err
if results:
...
st.dataframe(top_oversold[cols_o].rename(columns={'DisplayName': 'Symbol', 'C
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("##### Top 20 Most Overbought")
top_overbought = results_df.nlargest(20, 'Signal')
st.dataframe(top_overbought[cols_o].rename(columns={'DisplayName': 'Symbol',
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("##### Extreme Signals Chart")
st.plotly_chart(create_ranking_chart(results_df, 20), width="stretch", config
with tab3:
col_d1, col_d2 = st.columns(2)
with col_d1:
st.markdown("##### Signal Distribution")
...
st.markdown("##### Statistical Summary")
stats_data = {
"Metric": ["Total Stocks", "Mean Signal", "Median Signal", "Std Dev",
"Value": [
f"{len(results_df)}",
f"{results_df['Signal'].mean():.2f}",
f"{results_df['Signal'].median():.2f}",
...
col_bull, col_bear = st.columns(2)
with col_bull:
st.markdown('<span class="status-badge buy">BULLISH REGIME STOCKS</span>'
bull_stocks = results_df[results_df['Regime'].str.contains('BULL', na=Fal
...
with col_bear:
st.markdown('<span class="status-badge sell">BEARISH REGIME STOCKS</span>
bear_stocks = results_df[results_df['Regime'].str.contains('BEAR', na=Fal
...
st.markdown(f'<div class="symbol-row"><div><span class="symbol-name">
with tab5:
st.markdown(f"##### Complete Market Scan Results ({len(results_df)} stocks) -
# Filter options
filter_col1, filter_col2, filter_col3 = st.columns(3)
with filter_col1:
zone_filter = st.multiselect("Filter by Zone", ["Oversold", "Neutral", "O
...
display_cols = ['DisplayName', 'Price', 'Change', 'Signal', 'MSF', 'Zone', 'T
display_df = filtered_df[display_cols].copy()
display_df.columns = ['Symbol', 'Price', 'Chg %', 'Signal', 'MSF', 'Zone', 'T
...
label=" Download Full Report (CSV)",
data=csv_data,
file_name=f"nirnay_market_{universe_title.replace(' ', '_')}_{analysis_da
mime="text/csv"
)
else:
st.warning("No data retrieved. Please check your internet connection or try a dif
def run_market_timeseries_mode(length, roc_len, regime_sensitivity, base_weight, spread_unive
...
# Display info
universe_title = spread_index if spread_universe == "Index Constituents" and spread_index
st.markdown(f"""
<div class='info-box'>
<h4> </div>
Time Series Analysis - {universe_title}</h4>
<p>Full NIRNAY (MSF + MMR + Regime) signal distribution over time.<br>
<strong>Period:</strong> {start_date.strftime("%d %b %Y")} to {end_date.strftime("%d
""", unsafe_allow_html=True)
st.markdown("<br>", unsafe_allow_html=True)
...
status_text.markdown(f"** progress_bar.progress(0.05)
Downloading historical data for {total_stocks} stocks...*
data_dict, batch_msg = fetch_batch_data(stock_list, end_date=end_date, days_back=100
if data_dict is None:
st.error(f"Failed to download data: {batch_msg}")
progress_bar.empty()
status_text.empty()
...
trading_days = sample_df.index[(sample_df.index >= start_ts) & (sample_df.index <= en
...
is_today_included = actual_last_date == datetime.date.today() if actual_last_date els
if end_date == datetime.date.today():
if is_today_included:
st.toast(f"Live Data Included - {actual_last_date.strftime('%d %b %Y')}", ico
else:
st.toast(f"Data through {actual_last_date.strftime('%d %b %Y')}", icon=" elif actual_last_date and actual_last_date < end_date:
st.toast(f"Data through {actual_last_date.strftime('%d %b %Y')}", icon=" ")
")
...
df = df.join(macro_df, how='left').ffill()
# Run FULL analysis (MSF + MMR + Regime Intelligence)
df, _ = run_full_analysis(df, length, roc_len, regime_sensitivity, processed_data[ticker] = df
except Exception as e:
errors.append(f"{ticker}: {str(e)[:80]}")
base_w
if (i + 1) % 50 == 0:
...
day_stats["Oversold_Pct"] = (day_stats["Oversold"] / day_stats["Total_Analyze
day_stats["Overbought_Pct"] = (day_stats["Overbought"] / day_stats["Total_Ana
day_stats["Neutral_Pct"] = (day_stats["Neutral"] / day_stats["Total_Analyzed"
# NEW: Regime percentages
day_stats["Regime_Bull_Pct"] = (day_stats["Regime_Bull"] / day_stats["Total_A
day_stats["Regime_Bear_Pct"] = (day_stats["Regime_Bear"] / day_stats["Total_A
day_stats["Vol_High_Pct"] = (day_stats["Vol_High"] / day_stats["Total_Analyze
else:
...
st.toast(f"Time Series Complete! {len(ts_df)} days ({actual_start} to {actual_end})",
# Summary metrics
st.markdown("<br>", unsafe_allow_html=True)
c1, c2, c3, c4, c5, c6 = st.columns(6)
with c1:
avg_oversold = ts_df['Oversold_Pct'].mean()
st.markdown(f'<div class="metric-card success"><h4>Avg Oversold</h4><h2>{avg_over
with c2:
avg_overbought = ts_df['Overbought_Pct'].mean()
st.markdown(f'<div class="metric-card danger"><h4>Avg Overbought</h4><h2>{avg_ove
with c3:
total_buys = ts_df['Buy_Signals'].sum()
st.markdown(f'<div class="metric-card primary"><h4>Total Buy Signals</h4><h2>{tot
with c4:
total_sells = ts_df['Sell_Signals'].sum()
st.markdown(f'<div class="metric-card warning"><h4>Total Sell Signals</h4><h2>{to
with c5:
avg_signal = ts_df['Avg_Signal'].mean()
regime = "BULLISH" if avg_signal < -1 else "BEARISH" if avg_signal > 1 else "NEUT
regime_color = "success" if avg_signal < -1 else "danger" if avg_signal > 1 else
st.markdown(f'<div class="metric-card {regime_color}"><h4>Period Regime</h4><h2 s
with c6:
st.markdown(f'<div class="metric-card info"><h4>Trading Days</h4><h2>{len(ts_df)}
st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
# Tabs for different views
tab3, tab1, tab2, tab4 = st.tabs(["** Regime Analysis**", "** Zone Trends**", "**
with tab1:
st.markdown("##### Overbought / Oversold Distribution Over Time")
st.markdown('<p style="color: #888888; font-size: 0.85rem;">Shows the percentage
...
xaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)'),
yaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)', title='% of Stocks'
...
xaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)'),
yaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)', title='Stock legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1,
font=dict(family='Inter', color='#EAEAEA'), hovermode='x unified',
...
xaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)'),
yaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)', title='Divergence C
legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1,
font=dict(family='Inter', color='#EAEAEA'), hovermode='x unified',
...
st.markdown('<p style="color: #888888; font-size: 0.85rem;">Negative = Bullish Bi
fig_avg = go.Figure()
colors = ['#10b981' if v < -2 else '#ef4444' if v > 2 else '#888888' for v in ts_
...
fig_avg.add_hline(y=-2, line=dict(color='rgba(16,185,129,0.5)', width=1, dash='da
fig_avg.add_hline(y=0, line=dict(color='rgba(255,255,255,0.3)', width=1))
fig_avg.update_layout(
template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='#1A1A1A'
margin=dict(l=10, r=10, t=30, b=50),
xaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)'),
yaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)', title='Average Sign
legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1,
font=dict(family='Inter', color='#EAEAEA'), hovermode='x unified'
)
...
st.markdown('<p style="color: #888888; font-size: 0.85rem;">Percentage of stocks
# Regime trend chart
...
xaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)'),
yaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)', title='Count legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1,
font=dict(family='Inter', color='#EAEAEA'), hovermode='x unified'
/ %'),
)
st.plotly_chart(fig_vol, width="stretch", config={'displayModeBar': False})
...
regime_stats = {
"Metric": ["Avg Bull Regime %", "Avg Bear Regime %", "Total Change Points
"Value": [f"{avg_bull:.1f}%", f"{avg_bear:.1f}%", f"{int(total_changes)}"
}
st.dataframe(pd.DataFrame(regime_stats), width="stretch", hide_index=True)
with col_r2:
st.markdown("##### Signal Statistics")
signal_stats = {
"Metric": ["Mean Signal", "Median Signal", "Min Signal", "Max Signal", "S
"Value": [
...
'Buy Sig', 'Sell Sig', 'Avg Sig', 'Bull Regime', 'Bear Regim
...
data=csv_data,
mime="text/csv"
file_name=f"nirnay_market_timeseries_{universe_title.replace(' ', '_')}_{actu
)
def run_etf_timeseries_mode(length, roc_len, regime_sensitivity, base_weight, start_date, end
"""ETF Time Series Analysis: Track overbought/oversold signals over time for fixed ETF un
# Validate dates
if start_date is None or end_date is None:
st.error("Please select both start and end dates.")
return
...
<p>Track overbought/oversold signal distribution across {len(SCREENER_SYMBOLS)} ETFs
<strong>Period:</strong> {start_date.strftime("%d %b %Y")} to {end_date.strftime("%d
</div>
""", unsafe_allow_html=True)
st.markdown("<br>", unsafe_allow_html=True)
...
if df is not None and len(df) > length + 5:
try:
df, _ = run_full_analysis(df, length, roc_len, regime_sensitivity, df.index = pd.to_datetime(df.index)
if df.index.tz is not None:
df.index = df.index.tz_localize(None)
...
actual_start = ts_df['Date'].min().strftime('%d %b %Y')
actual_end = ts_df['Date'].max().strftime('%d %b %Y')
st.toast(f"ETF Time Series Complete! {len(ts_df)} days ({actual_start} to {actual_end
# Summary metrics
st.markdown("<br>", unsafe_allow_html=True)
c1, c2, c3, c4, c5, c6 = st.columns(6)
with c1:
...
margin=dict(l=10, r=10, t=30, b=50),
xaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)'),
yaxis=dict(showgrid=True, gridcolor='rgba(42,42,42,0.5)', title='% of ETFs',
legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1,
font=dict(family='Inter', color='#EAEAEA'), hovermode='x unified'
)
st.plotly_chart(fig_zones, width="stretch", config={'displayModeBar': False})
...
# NEW: HMM Regime Distribution Over Time
st.markdown("##### HMM Regime Distribution Over Time")
st.markdown('<p style="color: #888888; font-size: 0.85rem;">Percentage of ETFs in
# Regime trend chart
fig_regime = go.Figure()
fig_regime.add_trace(go.Scatter(
x=ts_df['Date'], y=ts_df['Regime_Bull_Pct'],
